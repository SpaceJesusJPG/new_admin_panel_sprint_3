...
class Runner:
    ...
    def run_etl_genre(self, new_modified_ls: List[List[Any]]):
        ...
        offset = 0
        while True:
            genre_fw_id_ls = extractor.enrich(
                new_modified_ls, GENRE_BATCH_SIZE, offset
            )
    ...
    def run_loop(self):
        while True:
            ...
            if new_modified["film_work"]:
                ...
                logging.info("Загрузка обновлений таблицы film_work в Elasticsearch завершена")
            if new_modified["person"]:
                logging.info("Получено обновление таблицы person")
                self.run_etl_person(new_modified["person"])
                logging.info("Загрузка обновлений таблицы person в Elasticsearch завершена")
            if new_modified["genre"]:
                logging.info("Получено обновление таблицы genre")
                self.run_etl_genre(new_modified["genre"])
                logging.info("Загрузка обновлений таблицы genre в Elasticsearch завершена")
